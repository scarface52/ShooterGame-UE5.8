...
void UShooterGameInstance::Init() 
{
	...
	IPlatformInputDeviceMapper::Get().GetOnInputDeviceConnectionChange().AddUObject(this, &UShooterGameInstance::HandleControllerConnectionChange);
...
void UShooterGameInstance::HandleControllerConnectionChange(EInputDeviceConnectionState ConnectionState, FPlatformUserId Unused, FInputDeviceId InputDeviceId)
{
	const bool bIsConnection = ConnectionState == EInputDeviceConnectionState::Connected;
	const int32 GameUserIndex = InputDeviceId.GetId();
...
void UShooterGameInstance::StartOnlinePrivilegeTask(const IOnlineIdentity::FOnGetUserPrivilegeCompleteDelegate& Delegate, EUserPrivileges::Type Privilege, TSharedPtr< const FUniqueNetId > UserId)
{
	...
	else
	{
		...
		Delegate.ExecuteIfBound(*FUniqueNetIdString::EmptyId(), Privilege, (uint32)IOnlineIdentity::EPrivilegeResults::NoFailures);
	}
}
